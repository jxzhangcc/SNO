...
def parse_47(filename, silent=False, h11flag=False):
    ...
    # data['population'] = np.array([tri2square(u, dim) for u in np.fromstring(d['$DENSITY'], sep = ' ').reshape(dim_spin, -1)])
    ...
    if '$FOCK' in d:
        # data['fock'] = tri2square(np.fromstring(d['$FOCK'], sep = ' '), dim_bs)
    ...
    if '$C10' in d:
        ...
        # data['C10'] = np.fromstring(d['$C10'], sep = ' ').reshape(3, dim_bs, -1).transpose(0,2,1)
        if not silent:
